...
class Summarization(Task):
    """
    Summarization task - one of the two tasks in the user edit environment
    Given an article from a specific source, summarize the article according to some initially unknown preferences.
    """
    ALL_DATASETS = ["cnn_dailymail", "slf5k", "wikipedia", "CShorten/ML-ArXiv-Papers", "imdb"]
    ...
    def __init__(self, config: SimpleNamespace):
        ...
        self.possible_preferences = list(ARTICLE_SOURCE_TO_PREFERENCE[framework].values())
    ...
    def get_task_instances(self) -> t.Iterable[WritingTaskInstance]:
        ...
        for d in self.data:
            ...
            yield WritingTaskInstance("summarization", d.article.strip(), d.doc_type, source_idx)
    ...
    def get_metrics(self, task_instance: WritingTaskInstance, pred_preferences: WritingPreferenceSet,
        ...
        metrics = {
            ...
            "preference_llm_judge_similarity": self.quality_assessor.score_preference_similarity(pred_preference=pred_preferences, true_preference=true_preferences),
