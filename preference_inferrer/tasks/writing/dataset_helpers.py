...
class OurInputDataset(Dataset):
    ...
    *_keys are string keys in each example dictionary in the given data loaded from hugging face
    ...
    def __init__(self, data, num_ex: int, id_key: str, article_key: str, doc_type: str = ''):
        ...
        self.dataset = [OurInputExample(example_id=d[id_key],
                                        article=d[article_key],
                                        doc_type=doc_type) for d in data]

    def __getitem__(self, item: int) -> OurInputExample:
...
def load_data(dataset: str, num_ex: int = -1, split: str = 'train') -> OurInputDataset:
    """
    ...
    if dataset == 'cnn_dailymail':
        ...
        data = load_dataset("JeremyAlain/SLF5K", trust_remote_code=True)[split]
        filtered_data = []
        for ex in data:
            length = len(ex['post'].split())
            if length > 150:  # 4809
        ...
        data = load_dataset(dataset, '20220301.simple', trust_remote_code=True)[split]
        filtered_data = []
        for ex in data:
            length = len(ex['text'].split())
            if 500 < length < 800:  # 4809
                filtered_data.append(ex)
        ...
        for ex in data:
            ...
            if length > 150:  # 4809
                filtered_data.append(ex)
        # note that the label is not actually ids
        parsed_data = OurInputDataset(data=filtered_data, num_ex=num_ex, id_key='label', article_key='text',
                                      doc_type=dataset)
    elif dataset == 'ccby':
        data = load_dataset('orieg/elsevier-oa-cc-by', trust_remote_code=True)['train']
        filtered_data = []
        for ex in data:
